...
object fp3:
  ...
  def removeDupes2[X](xs: List[X]): List[(Int, X)] =
    ...
    xs match{
      case Nil => Nil
      case y::Nil => List((1,y))
      case y::ys if(y==ys.head) =>  {val t = removeDupes2(ys); t.updated(0,(t(0)._1+1,y))}
      case y::ys => (1,y)::removeDupes2(ys)
    }
  ...
  def allDistinct(xs: List[Int]): Boolean =
    ...
    xs match {
      case Nil => true
      case y :: ys => if (member(y, ys) == true) then
        false
       else{
        allDistinct(ys)
      }
